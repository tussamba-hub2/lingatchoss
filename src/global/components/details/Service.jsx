...
import HomeServices from "../datas/HomeServices";
...
export default function Service() {
  ...
  function handle_whastapp(name, company_id) {
    // Prefixa o company_id antes da mensagem
    const language = localStorage.getItem("language") || "pt"; // fallback para 'pt'
    const fullMessage = `${company_id} ${language} ${t("greetings_service")} ${name}`;


    // Substitui o número pelo número de teste do WABA
    const test_number = "+244935150370"; // Número de teste do WhatsApp Business API

    // Gera a URL do WhatsApp
    const url = `https://wa.me/${test_number}?text=${encodeURIComponent(
      fullMessage
    )}`;
  ...
  return (
    <div className="d-flex column g-36px hover-contenting">
      <div className="padding-inside mt-100 d-flex items-center justify-center rs-mt-0 column">
        <div className="service-details relative">
          <div className="img-svc-dtls relative">
            <img
              src={service.image_url || "../assets/images/app-logo.png"}
              alt={service.name}
            />
          ...
          <div className="d-flex column g-32px p-16">
            ...
            <div className="d-flex column g-12px">
              <NavLink className="d-flex items-center g-12px">
                <img
                  src={
                    service.user?.logo_url || "../assets/images/app-logo.png"
                  }
            ...
            <button
              onClick={() => handle_whastapp(service.name, service.user_id)}
